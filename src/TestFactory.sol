...
contract TestFactory is Setup {
    ...
    function createNEwDaoMemeber() public {
        ...
        tierConfigs[0] = TierConfig({amount: 50, minted: 0, price: 1, power: 0});
        tierConfigs[1] = TierConfig({amount: 50, minted: 0, price: 1, power: 0});
        tierConfigs[2] = TierConfig({amount: 50, minted: 0, price: 1, power: 0});
    ...
    function test_JoinDao() public {
        if (isSetup) {
            (, DAOType daoType, address _currency, uint256 maxMembers, uint256 noOfTiers) = factory.daos(newDao);
            assert(_currency == address(currency));
            currency.approve(address(factory), type(uint256).max);
            _joinDao(noOfTiers - 1);
            uint256 bal = MembershipERC1155(newDao).balanceOf(address(this), noOfTiers - 1);
            assert(bal >= 1);
            isMinted = true;
        }
    }
    ...
    function test_upgradeTier() public {
        if (isMinted) {
            ...
            if (bal > 0) {
                ...
                uint256 bal1 = MembershipERC1155(newDao).balanceOf(address(this), noOfTiers - 1 - 1);
                assert(bal1 >= 1);
            }
        }
    }
